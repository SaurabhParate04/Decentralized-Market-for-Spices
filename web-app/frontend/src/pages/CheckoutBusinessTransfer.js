...
export default function CheckoutBusinessTransfer(props) {
    ...
    const invoke = async(func, prodId, objp, channel) => {
        try {
            console.log('invoke from CheckoutBusinessTransfer' + func, prodId)
    ...
    const addWholesaler = async() => {
        const url = "http://localhost:5000/api/manufacturedproduct/updateproductbyid/" + id;
        ...
        const response = await fetch(url,
            {
                ...
                body: JSON.stringify({
                    wholesaler: userProfileBusiness.username
                })
            }
        );
    }
        ...
        const response = await fetch(url,
            {
                ...
                body: JSON.stringify({
                    retailer: userProfileBusiness.username
                })
            }
        );
    }
    ...
    const query = async(prodId, channel) => {
        try {
            console.log('query from CheckoutBusinessTransfer' + prodId)
                                ...
                                <table className="ct-responsive-table">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td data-title="Quantity"> <input type="number" name="quantity" min={total} max={total} required/> Pcs</td>
